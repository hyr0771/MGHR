        ...
        match split:
            ...
            case "testdev":
                ...
                self.data = {
                    ...
                    if (            # 检查每个问题的 imageId 是否存在于 self.sg_feature_lookup.scene_graphs_testdev 字典的键中,
                                        # 只有在 imageId 存在于 scene_graphs_testdev 中时，问题才会被保留
                        value["imageId"]
                        in self.sg_feature_lookup.scene_graphs_testdev.keys()
                    )
                    and (       #确保对应的 scene_graphs_testdev 中的图像数据不是 None，
                                # 只有在该图像的数据有效时，问题才会被保留。
