...
from ..datasets.scene_graph import GQASceneGraphs, VQASceneGraphs


class SceneGraphEncoder(torch.nn.Module):
    def __init__(self, hidden_dim, dist=False,dataset="GQA"):
        super(SceneGraphEncoder, self).__init__()
        if dataset == "GQA":
            self.scene_graphs_helper = GQASceneGraphs()
        elif dataset == "VQA":
            self.scene_graphs_helper = VQASceneGraphs()
        else:
            raise NotImplementedError("Unknown dataset")
        self.sg_vocab = self.scene_graphs_helper.vocab_sg
        self.hidden_dim = hidden_dim
        # 允许通过环境变量或外部参数禁用 SyncBatchNorm（调试多卡不同步/死锁）
        disable_syncbn_env = os.getenv('DISABLE_SYNCBN', '0')
        if disable_syncbn_env.lower() in ('1', 'true', 'yes'):
            dist = False
        self.dist = dist
