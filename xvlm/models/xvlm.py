...
from xvlm.models import box_ops
from xvlm.models.vit import VisionTransformer, interpolate_pos_embed
from xvlm.models.clip_vit import CLIPVisionTransformer
from xvlm.models.swin_transformer import SwinTransformer, interpolate_relative_pos_embed

from xvlm.models.xbert import BertConfig, BertForMaskedLM, BertModel
from xvlm.models.xroberta import RobertaConfig, RobertaForMaskedLM, RobertaModel
...
def load_pretrained(ckpt_rpath, config, is_eval=False, load_text=False):
    ...
    参数:
        ckpt_rpath (str): 检查点文件的路径。
        config (dict): 模型的配置。
        is_eval (bool, 可选): 模型是否处于评估模式，默认为 False。
        load_text (bool, 可选): 是否加载与文本相关的权重，默认为 False。
    """
    checkpoint = torch.load(ckpt_rpath, map_location='cpu') # 加载检查点文件
    state_dict = checkpoint['model'] if 'model' in checkpoint.keys() else checkpoint     # 从检查点中提取状态字典

    if is_eval: # 如果模型处于评估模式，则不需要进一步处理
        return state_dict

    num_patches = (config['image_res'] // config['patch_size']) ** 2        # 从模型配置中获取patch数量

    print("### Loading pretrained vision encoder", flush=True)      # 加载预训练的视觉编码器权重
    if config['use_clip_vit']:      # 对于 CLIP 视觉变换器，进行位置嵌入的插值
    ...
    elif config['use_swin']:        # 对于 Swin 变换器，进行相对位置偏置表的插值
    ...
    if load_text:       # 如果指定加载与文本相关的权重，则加载预训练的文本编码器权重
