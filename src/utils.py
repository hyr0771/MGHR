...
def trim_collate(batch):
    # for features, spatials, stat_features, entity, question_ban, target, ans, image, question_xvlm, question_id in batch:

    "Puts each data field into a tensor with outer dimension batch size"
    _use_shared_memory = True
    error_msg = "batch must contain tensors, numbers, dicts or lists; found {}"
    elem_type = type(batch[0])       # 获取batch中第一个元素的类型

    if torch.is_tensor(batch[0]) :   # 如果输入的是tensor类型数据
        out = None

        if 3 == batch[0].dim(): # image xvlm        # 如果是3维tensor(图像数据)
            return torch.stack(batch, 0, out=out)   # 直接堆叠

        elif 2 == batch[0].dim(): # image features   # 如果是特征数据(维度>1)
            max_num_boxes = max([x.size(0) for x in batch]) # 找出batch中最大的box数量
            if _use_shared_memory:  # 在后台进程中,直接使用共享内存来避免额外的复制操作
                # If we're in a background process, concatenate directly into a
                # shared memory tensor to avoid an extra copy
                numel = len(batch) * max_num_boxes * batch[0].size(-1)
                storage = batch[0].storage()._new_shared(numel)
                out = batch[0].new(storage)
            # warning: F.pad returns Variable!       # 对每个样本进行补齐,使其具有相同的box数量,然后堆叠
            ...
            and elem_type.__name__ != 'string_':    # 如果输入是numpy数组
        elem = batch[0]
        if elem_type.__name__ == 'ndarray':
            # array of string classes and object        # 检查数组的数据类型,不接受字符串类型和对象
            if re.search('[SaUO]', elem.dtype.str) is not None:
                raise TypeError(error_msg.format(elem.dtype))

            return torch.stack([torch.from_numpy(b) for b in batch], 0)     # 将numpy数组转换为tensor并堆叠
        if elem.shape == ():  # scalars     # 如果是标量
            py_type = float if elem.dtype.name.startswith('float') else int     # 根据数据类型选择转换为float或int
            return numpy_type_map[elem.dtype.name](list(map(py_type, batch)))
    elif isinstance(batch[0], int):  # 如果输入是整数
        return torch.LongTensor(batch)
    elif isinstance(batch[0], float):   # 如果输入是浮点数
        return torch.DoubleTensor(batch)
    elif isinstance(batch[0], string_classes):  # 如果输入是字符串
        return batch
    elif isinstance(batch[0], torch_geometric.data.Data):
        return torch_geometric.data.Batch.from_data_list(batch)
    elif isinstance(batch[0], collections.abc.Mapping): # 如果输入是字典类型
        # return {key: default_collate([d[key] for d in batch]) for key in batch[0]}
        for d in batch:
            for key, value in d.items():
                print(f"Key: {key}, Value: {value}")
        return batch
    elif isinstance(batch[0], collections.abc.Sequence):    # 如果输入是序列类型(list, tuple等)
        transposed = zip(*batch)
        return [trim_collate(samples) for samples in transposed]

    raise TypeError((error_msg.format(type(batch[0]))))     # 如果输入类型不在上述任何一种中,抛出类型错误
